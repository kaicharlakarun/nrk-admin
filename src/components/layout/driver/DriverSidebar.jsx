...
import { Zap, Car,Home, Wrench } from "lucide-react";

const DriverSidebar = ({ sidebarOpen, setSidebarOpen, activePage, setActivePage }) => {
  const menuItems = [
    { name: "Dashboard", icon: <Home className="w-5 h-5 mr-3" /> },
    { name: "Trips", icon: <Car className="w-5 h-5 mr-3" /> },
  ...
  <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg bg-white">
    ...
    />
            ...
            <div>
              <h1 className="text-xl font-bold text-green-500">NRK Travels</h1>
              <p className="text-xs text-green-500">Driver Panel</p>
            </div>
          </div>
            ...
            <button
              ...
              className={`flex items-center w-full p-2 rounded-lg transition-colors ${
                activePage === item.name
                  ? "bg-green-500 text-white" // selected state: green background + white text
                  : "text-green-500 hover:bg-green-300 hover:text-white" // unselected: green text + hover effect
              }`}
            >
              {item.icon} {item.name}
            </button>
