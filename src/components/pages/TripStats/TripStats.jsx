...
const TripStats = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const res = await fetch(`${BASE_URL}/api/trips/stats?month=${currentMonth}`, {
  ...
  const DashboardCards = [
    ...
    { title: "Profit", value: `₹${formatNumber(stats.totalProfit)}`, 
      bg: stats.totalProfit >= 0 ? "bg-green-700" : "bg-gradient-to-r from-red-400 to-red-600" },
  ];
      ...
      {loading ? (
        ...
        DashboardCards.map((card, index) => (
          ...
          >
            ...
            <p className={`mt-2 text-2xl font-bold break-words ${card.title === "Profit" && stats.totalProfit < 0 ? "text-red-100" : "text-white"}`}>
              {card.value}
            </p>
          </div>
        ))
